...
const defaultErrorMessageByStatus = (statusCode) => {
  ...
  if (statusCode === 401) return 'غير مصرح لك بتنفيذ هذا الطلب.';
  if (statusCode === 403) return 'ليس لديك صلاحية للوصول إلى هذا المورد.';
  if (statusCode === 404) return 'العنصر المطلوب غير موجود.';
  if (statusCode === 409) return 'تعذر تنفيذ الطلب بسبب تعارض في البيانات.';
  return 'حدث خطأ غير متوقع. حاول مرة أخرى.';
};

export const toArabicErrorMessage = (message, statusCode = 500) => {
  if (!message) return defaultErrorMessageByStatus(statusCode);

  const normalized = String(message).trim();
  if (!normalized) return defaultErrorMessageByStatus(statusCode);
...
export const successResponse = (res, { message = 'تمت العملية بنجاح.', data = null, statusCode = 200 }) =>
